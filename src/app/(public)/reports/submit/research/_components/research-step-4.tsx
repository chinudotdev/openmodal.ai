...
interface ResearchStep4Props {
  // @ts-expect-error - TanStack Form type compatibility
  form: FormApi<Partial<ResearchReportInput>>;
  onBack: () => void;
  formData: ResearchReportInput;
}

export function ResearchStep4({
  form,
  onBack,
  formData: _formData,
}: ResearchStep4Props) {
  // Subscribe to form state changes to ensure we have the latest data
  const [formData, setFormData] = useState<ResearchReportInput>(
    form.state.values as ResearchReportInput,
  );

  useEffect(() => {
    const subscription = form.store.subscribe((state: any) => {
      if (state.values) {
        setFormData(state.values as ResearchReportInput);
      }
    });
    return () => subscription();
  }, [form]);
          ...
          <CardContent className="space-y-2">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm">
                  {formData.step1?.capabilityName || "N/A"}
                </p>
              </div>
              <div>
                <p className="text-sm font-medium text-muted-foreground">
              ...
              <div>
                ...
                <p className="text-sm">
                  {formData.step1?.organization || "Not specified"}
                </p>
              </div>
              <div>
                <p className="text-sm font-medium text-muted-foreground">
          ...
          <CardContent className="space-y-2">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm">
                  {formatResearchType(formData.step2?.researchType)}
                </p>
              </div>
              <div>
                <p className="text-sm font-medium text-muted-foreground">
                ...
                <p className="text-sm">
                  {formData.step2?.publicationDate
                    ? new Date(
                        formData.step2.publicationDate,
            ...
            <div>
              ...
              <p className="text-sm whitespace-pre-wrap">
                {formData.step2?.impactDescription || "N/A"}
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent className="pt-6">
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              <Checkbox id="research_guideline1" required />
              <Label htmlFor="research_guideline1" className="text-sm">
                I confirm this information is accurate
              </Label>
            </div>
            <div className="flex items-start gap-3">
              <Checkbox id="research_guideline2" required />
              <Label htmlFor="research_guideline2" className="text-sm">
                I have evidence to support my claims
              </Label>
            </div>
            <div className="flex items-start gap-3">
              <Checkbox id="research_guideline3" required />
              <Label htmlFor="research_guideline3" className="text-sm">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              You'll earn +50 reputation points once approved!
            </p>
          </div>
        </div>
      </div>
