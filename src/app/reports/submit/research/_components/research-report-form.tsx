...
import { ResearchStep1 } from "./research-step-1";
import { ResearchStep2 } from "./research-step-2";
import { ResearchStep3 } from "./research-step-3";
import { ResearchStep4 } from "./research-step-4";
...
export function ResearchReportForm() {
  ...
  const [formData, setFormData] = useState<Partial<ResearchReportInput>>({
    type: "research",
    isDraft: false,
  });
  ...
  const form = useForm({
    ...
    validators: {
      onSubmit: researchReportSchema as any,
    },
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      const result = await submitReport(
        user.id,
        currentData as ResearchReportInput,
      );
      if (result.success) {
  ...
  const validateStep = (
    ...
    switch (step) {
      case 1:
        return !!(
          (data.step1 as Record<string, unknown>)?.capabilityName &&
          (data.step1 as Record<string, unknown>)?.technology
        );
      case 2:
        return !!(
          (data.step2 as Record<string, unknown>)?.researchType &&
          (data.step2 as Record<string, unknown>)?.impactDescription
        );
      case 3: {
  ...
  const updateFormData = (step: number, data: Record<string, unknown>) => {
    ...
    if (step === 1) newData.step1 = data as ResearchReportInput["step1"];
    if (step === 2) newData.step2 = data as ResearchReportInput["step2"];
    if (step === 3) newData.step3 = data as ResearchReportInput["step3"];
    setFormData(newData);
      ...
      <CardContent>
        ...
        {currentStep === 4 && (
          <ResearchStep4
            form={form}
            onBack={handleBack}
            formData={formData as ResearchReportInput}
          />
        )}
